...
SCORE_TXT = 'highest_rs_score.txt'
...
def get_high_scores():
    high_scores = []
    try:
        with open("rs_scores.txt", "r") as file:
            for line in file:
...
user_name = st.text_input('What name would you like to display in the leaderboard?', max_chars=50, key='user_name',  value='')

if user_name:
    st.session_state.questions = get_questions('pages/hidethis/intent.json')
    ...
    if 'questions' not in st.session_state:
        st.session_state.questions = get_questions('intent.json')
